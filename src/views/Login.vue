<template>
    ...
    <form class="mt-8 space-y-6" @submit="login">
        <div v-if="errorMsg" class="flex items-center justify-between py-3 px-5 bg-red-500 text-white rounded">
            {{ errorMsg }}
            <span @click="errorMsg = ''" class="w-8 h-8 flex items-center justify-center rounded-full transition-colors cursor-pointer hover:bg-(0,0,0,0.2)">
...
function login(ev) {
    ev.preventDefault()
    store.dispatch("login", user).then(() => { router.push({ name: "Dashboard" }) }).catch(err => { errorMsg.value = err.response.data.error })
}
</script>